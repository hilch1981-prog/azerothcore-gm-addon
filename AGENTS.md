...
5. 모바일 Remote에서 짧은 지시로 작업을 시작한 경우 `docs/project/MOBILE_REMOTE_WORKFLOW.md`를 추가로 읽고 그 절차를 따른다.

## 모바일 Remote 작업 규칙

사용자가 모바일에서 `PR #50 계속`, `다음`, `테스트 ZIP 만들어`, `Claude 지적 확인`처럼 짧게 지시해도 다음을 자동으로 수행한다.

1. 현재 checkout 브랜치와 HEAD commit SHA를 확인한다.
2. `origin/main` 및 관련 PR/태그와의 ahead/behind 또는 diverged 상태를 확인한다.
3. 관련 Issue/PR, `PROJECT_STATUS.md`, `TASKS.md`의 현재 상태를 대조한다.
4. 공개 릴리즈 ZIP과 PR 검증 ZIP을 혼동할 가능성이 있으면 작업 전에 명확히 구분한다.
5. 사용자가 이미 제공한 정보는 다시 묻지 않는다.
6. 안전하게 진행 가능한 분석·정적 검증은 계속 수행하고, merge/release/tag 같은 비가역 작업만 명시적 승인 전에는 실행하지 않는다.
7. 모바일 응답은 긴 로그보다 `현재 상태 / 작업 대상 / 발견 문제 / 수정 여부 / 자동 테스트 / Claude 리뷰 / 게임 테스트 필요 / GitHub 반영 상태` 순서의 짧은 요약을 우선한다.
...
- 사용자의 명시적 승인 전에는 PR merge, release 생성, tag 생성, Issue 종료를 실행하지 않는다.
...
- 게임 테스트 ZIP은 PR 번호와 commit SHA를 식별할 수 있는 이름을 사용하고, 공개 Release ZIP과 별도로 관리한다.
