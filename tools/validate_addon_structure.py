...
ORDER_RULES = (
    ...
    ("KoKRSearchData.lua", "KoKRSearch.lua"),
    ("KoKRSearchData.lua", "FeaturedCreatures.lua"),
    ("FeaturedCreatures.lua", "CreatureBrowser.lua"),
...
def validate() -> list[str]:
    ...
    if "## Interface: 30300" not in toc_text:
        errors.append("TOC Interface must be 30300")

    duplicates = sorted({entry for entry in entries if entries.count(entry) > 1})
    if duplicates:
        errors.append("duplicate TOC entries: " + ", ".join(duplicates))

    for entry in entries:
        if resolve_exact(entry) is None:
            errors.append(f"missing or case-mismatched TOC path: {entry}")

    for before, after in ORDER_RULES:
        if before not in positions:
            errors.append(f"load-order source missing: {before}")
        elif after not in positions:
            errors.append(f"load-order target missing: {after}")
        elif positions[before] >= positions[after]:
            errors.append(f"invalid load order: {before} must precede {after}")
    ...
    for snippet in (
        ...
        if snippet not in quest_source:
            errors.append(f"missing quest selection lifecycle: {snippet}")
